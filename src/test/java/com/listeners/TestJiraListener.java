...
public class TestJiraListener implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		 ...
		 * JiraPolicy.class); boolean isTicketReady = jpolicy.logTicketReady();
		 * 
		 * if (isTicketReady) { // raise a jira ticket
		 * System.out.println("Is ticket ready for Jira=" + isTicketReady);
		 * JiraServiceProvider jirasp = new
		 * JiraServiceProvider("https://sourcebits.atlassian.net/",
		 * "raviraj.metri@ascendum.com", "dTlpeqJYj5FiE8axqdC9E798", "NORBUG");
		 * 
		 * // NvISwviiwuCBJcuEfflw1BBE
		 * 
		 * // Summary of the issue String issueSummary =
		 * result.getMethod().getConstructorOrMethod().getMethod().getName() +
		 * "got failed due to some assertion / exception";
		 * 
		 * String issueDescription = result.getThrowable().getMessage() + "\n"; String
		 * issueDescription2 = ExceptionUtils.getFullStackTrace(result.getThrowable());
