...
public class TC_RuntimeMinmax_TemperatureTest extends TC_LoginTest_DDT_001 {
	...
	public void TestRuntimeMinmaxGlobalPointValidations(String lessminval, String moremaxval, String Inrng, String lessfhminval,
			String maxplusfhval, String Inrngfh) throws InterruptedException, IOException {
		...
		log.info("Tank set point for global Inrng point is set to :" + Inrng
				+ "for celsius unit");

		String manifoldforInrng = Trs.getManifold();
		softAssert.assertEquals(Inrng, manifoldforInrng,
				"Manifold set point is not set to Default value : " + Inrng
						+ "for celsius unit");
		...
		for (int i = 0; i < hosestpntInrng.size(); i++) {
			if (hosestpntInrng.get(i).equals(Inrng)) {
				log.info("Hose" + count + "set point temperature for global Inrng value set to"
						+Inrng + "for celsius unit");

			}

			else

				log.info("Hose" + count + "set point temperature for global Inrng value not set to"
						+ Inrng + "for celsius unit");
			count++;
		}
	...
	// Validations for Farnheit Unit for Global Set Point
		...
		softAssert.assertEquals(Constants.MaxsetglobalManifoldpntFarh, manifoldforgpmaxfh,
				"Manifold set point is not set to Default value : " + Constants.MaxsetglobalManifoldpntFarh
						+ "for farnhenit unit");
		log.info(
				"Manifold set point is set to Maximum value :" + Constants.MaxsetglobalManifoldpntFarh + "for farnhenit unit");
		Thread.sleep(1500);
		...
		log.info("Tank set point for global Inrng point is set to :" + Inrngfh
				+ "for farnhenit unit");

		String manifoldforInrngfh = Trs.getManifold();
		softAssert.assertEquals(Inrngfh, manifoldforInrngfh,
				"Manifold set point is not set to Default value : " + Inrngfh
						+ "for farnhenit unit");
		...
		XLUtils.setExcelSheetNm("TankPoint");
	...
	public void TestRuntimeMinmaxTankval(String minlsvalue, String maxplusval, String Inrngval, String minlsvaluefh,
			String maxplusvalfh, String Inrngvaluefh) throws InterruptedException, IOException {
		...
		softAssert.assertEquals(true, tstmsgstus, "Toast msg for In range values are saved successfully for Celsius unit");
		...
		softAssert.assertEquals(true, tstnullerrfhmsg, "Toast Error message is shown up for farnheit unit null val save");
		...
		softAssert.assertEquals(true, tstmaxerrfh,
				"Toast Error message is shown up for " + maxplusvalfh + " val for Celsius unit");
		...
		softAssert.assertEquals(true, stusmaxerrfh,
				"Error message is shown up for " + maxplusvalfh + " val for Celsius unit");
		...
		softAssert.assertEquals(true, tstfh, " Toast msg for In range values are saved successfully for Farnheit unit");
		...
		XLUtils.setExcelSheetNm("Hose");

	}

	@Test(priority = 3, dataProvider = "ReadDP", dataProviderClass = com.nordson.utilities.XLUtils.class)
	public void TestRuntimeMinmaxHoseval(String minlsvalue, String maxplusval, String Inrngval, String minlsvaluefh,
			String maxplusvalfh, String Inrngvaluefh) throws InterruptedException, IOException {
		...
		boolean tstmaxerr = Trs.getToastmsg();
		softAssert.assertEquals(true, tstmaxerr, "Toast Error message is  not shown up for max plus val for Celsius unit");
		...
		// checking the status and enabling Applicator
				...
				softAssert.assertEquals(true, appclrerrmsgtst, "Toast Error message is not shown up for max Celsius val for Applicator1");
				log.info("Toast Error message is shown up for null Celsius val save for Applicator1");
				Thread.sleep(3000);

				boolean appclrerrmsg = Trs.getbtnstatus();
				softAssert.assertEquals(true, appclrerrmsg, "Error message is not shown up for max Celsius val for Applicator1");
				...
				softAssert.assertEquals(true, apptstmaxerr, "Toast Error message is  not shown up for max plus val for Celsius unit Applicator1");
				...
				softAssert.assertEquals(true, appstusmaxerr, "Error message is  not shown up for max plus val for Celsius unit Applicator1");
				...
				softAssert.assertEquals(true, apptstsucssmsg, "Toast msg In range values are not saved for Celsius unit Applicator1");
		...
		Trs.clickPreferencesBtn();
		Thread.sleep(2200);
		log.info("Preferences is clicked to opt Unit farnheit temperature");

		Trs.clickFarhenitUnit();
		log.info("Temperature Unit farnheit is selected by clicking");
		Thread.sleep(3200);
		...
		Trs.ZoneTemperatureBtn();
		Thread.sleep(1200);
		log.info("Clicked on Zone temperature button to setglobal min value in farnheit");
		Thread.sleep(1200);
		...
		boolean tstmaxerrfh = Trs.getToastmsg();
		softAssert.assertEquals(true, tstmaxerrfh, "Toast error message is  not shown up for max plus val for Celsius unit");
		log.info("Toast error message is shown up for max val plus " + maxplusvalfh + " for Celsius unit");
		Thread.sleep(2000);


		boolean stusmaxerrfh = Trs.getbtnstatus();
		softAssert.assertEquals(true, stusmaxerrfh, "Error message is  not shown up for max plus val for Celsius unit");
		log.info("Error message is shown up for max val plus " + maxplusvalfh + " for Celsius unit");
		Thread.sleep(2000);
	...
	public static String[][] getDataTemp() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "GlobalPoint");
		int colcount = XLUtils.getCellCount(path, "GlobalPoint", 1);
		...
		for (int i = 1; i <= rownum; i++) {
			for (int j = 1; j <= colcnt; j++) {
				tempdata[i - 1][j - 1] = XLUtils.getCellData(path, "GlobalPoint", i, j);// 1 1
